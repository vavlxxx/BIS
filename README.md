...
Для автоматизации можно использовать утилиту `make`. После чего нужно проверить, правильно ли всё заполнено.

```bash
make init
```

После настройки переменных окружения необходимо собрать и запустить контейнеры.
...
Для определения города через Dadata ключи тоже нужно хранить в `.env.wordpress`. Их не нужно хардкодить в теме или класть в шаблоны.
...
Для работы с hCaptcha используется плагин `hCaptcha для WP`, в котором вписываются `secrectkey` и `sitekey`.
